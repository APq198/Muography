...
void DetectorConstruction::ConstructOmniDetector()
{
	...
	#else
		G4VPhysicalVolume * physDetector0 = new G4PVPlacement(0, G4ThreeVector(0., -detectorHeight/2.0 -yWorld, 0.), logicDetector, "physDetector0", logicWorld, false, 32, true);
	#endif
}
...
G4VPhysicalVolume * DetectorConstruction::ConstructSurfaceScene()
{
	...
	G4double zWorld = 20*km;
		...
		G4Box * solidWorld = new G4Box("solidWorld", xWorld, yWorld+2*detectorHeight, zWorld);
